...
function create_miami_plot(male_variant_bins, male_unbinned_variants, female_variant_bins, female_unbinned_variants){
    ...
    // flip p-values of female negative so it can be shown on the opposite side
    // female_variant_bins.forEach(function(bin){
    //     bin.qvals = bin.qvals.map(value => -value);
    //     bin.qval_extents = bin.qval_extents.map(sub_bin => sub_bin.map(value => -value));
    // })
    ...
    $(function() {
        ...
        var genomic_position_extent = (function() { //usually will be the same between males and females, so just choose males here.
            var extent1 = d3.extent(male_variant_bins, get_genomic_position);
            var extent2 = d3.extent(male_unbinned_variants, get_genomic_position);
            return d3.extent(extent1.concat(extent2));
        })();
        ...
        var y_axis_config_male = get_y_axis_config(Math.max(highest_plot_qval_female,highest_plot_qval_male), plot_height, includes_pval0, "upper");
        var y_scale_male = y_axis_config_male.scale;

        var y_axis_config_female = get_y_axis_config(Math.max(highest_plot_qval_female,highest_plot_qval_male), plot_height, includes_pval0, "lower");
