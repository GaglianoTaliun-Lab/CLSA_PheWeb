...
function create_miami_plot({male_variant_bins = null, male_unbinned_variants = null, female_variant_bins = null, female_unbinned_variants = null} = {}){

    // Order from weakest to strongest pvalue, so that the strongest variant will be on top (z-order) and easily hoverable
    // In the DOM, later siblings are displayed over top of (and occluding) earlier siblings.
    male_unbinned_variants = _.sortBy(male_unbinned_variants, function(d){return -d.pval});
    female_unbinned_variants = _.sortBy(female_unbinned_variants, function(d){return -d.pval})
    
    if (male_variant_bins != null ){

        if (male_variant_bins.length && typeof male_variant_bins[0].qvals === "undefined") {
            // this json was generated by an old version of pheweb, so we'll manually fix things up.
            male_variant_bins.forEach(function(bin) {
                bin.qvals = bin.neglog10_pvals;
                bin.qval_extents = bin.neglog10_pval_extents;
            });
        }
        var get_chrom_offsets_male = _.memoize(function() {
            var chrom_padding = 2e7;
            var chrom_extents = {};
            ...
            male_variant_bins.forEach(update_chrom_extents);
            male_unbinned_variants.forEach(update_chrom_extents);
    ...
    if (female_variant_bins != null){
        if (female_variant_bins.length && typeof female_variant_bins[0].qvals === "undefined") {
            // this json was generated by an old version of pheweb, so we'll manually fix things up.
            female_variant_bins.forEach(function(bin) {
                bin.qvals = bin.neglog10_pvals;
                bin.qval_extents = bin.neglog10_pval_extents;
            });
        }
    
        var get_chrom_offsets_female = _.memoize(function() {
            var chrom_padding = 2e7;
            var chrom_extents = {};
            ...
            female_variant_bins.forEach(update_chrom_extents);
            female_unbinned_variants.forEach(update_chrom_extents);
    ...
    function get_genomic_position_male(variant) {
        var chrom_offsets = get_chrom_offsets_male().chrom_offsets;
        return chrom_offsets[variant.chrom] + variant.pos;
    }

    function get_genomic_position_female(variant) {
        var chrom_offsets = get_chrom_offsets_female().chrom_offsets;
        return chrom_offsets[variant.chrom] + variant.pos;
    }
    ...
    $(function() {
        var svg_width = $('#miami_plot_container').width();
        var svg_height = 550;
        var plot_margin = {
            'left': 70,
            'right': 30,
        ...
        var miami_svg = d3.select('#miami_plot_container').append("svg")
            .attr('id', 'miami_svg')
            .attr("width", svg_width)
            .attr("height", svg_height)
            .style("display", "block")
        ...
        // default to male if its available.
        if (male_variant_bins != null){
            
            var genomic_position_extent = (function() { 
                var extent1 = d3.extent(male_variant_bins, get_genomic_position_male);
                var extent2 = d3.extent(male_unbinned_variants, get_genomic_position_male);
                return d3.extent(extent1.concat(extent2));
            })();
        }
        else if ( female_variant_bins != null){
            var genomic_position_extent = (function() { 
                var extent1 = d3.extent(female_variant_bins, get_genomic_position_female);
                var extent2 = d3.extent(female_unbinned_variants, get_genomic_position_female);
                return d3.extent(extent1.concat(extent2));
            })();
        }
        ...
        var includes_pval0 = _.any(male_unbinned_variants, function(variant) { return variant.pval === 0; }) ||  _.any(female_unbinned_variants, function(variant) { return variant.pval === 0; });

        //for y axis stuff, it will differ between males and famales, so we need to seperate them
        var highest_plot_qval_male = Math.max(
            -Math.log10(significance_threshold) + 0.5,
            (function() {
                var best_unbinned_qval = -Math.log10(d3.min(male_unbinned_variants, function(d) {
                    return (d.pval === 0) ? 1 : d.pval;
                }));
                if (best_unbinned_qval !== undefined) return best_unbinned_qval;
                return d3.max(male_variant_bins, function(bin) {
                    return d3.max(bin, _.property('qval'));
                });
            })());
        
        var highest_plot_qval_female = Math.max(
            -Math.log10(significance_threshold) + 0.5,
            (function() {
                var best_unbinned_qval = -Math.log10(d3.min(female_unbinned_variants, function(d) {
                    return (d.pval === 0) ? 1 : d.pval;
                }));
                if (best_unbinned_qval !== undefined) return best_unbinned_qval;
                return d3.max(female_variant_bins, function(bin) {
                    return d3.max(bin, _.property('qval'));
                });
            })());

        if (!isNaN(highest_plot_qval_female) && !isNaN(highest_plot_qval_male)){
            var max_value = Math.max(highest_plot_qval_female,highest_plot_qval_male);
        } else if (isNaN(highest_plot_qval_male)){
            var max_value = highest_plot_qval_female
        } else if (isNaN(highest_plot_qval_female)){
            var max_value = highest_plot_qval_male
        }

        var y_axis_config_male = get_y_axis_config(max_value, plot_height, includes_pval0, "upper");
        var y_scale_male = y_axis_config_male.scale;
        
        var y_axis_config_female = get_y_axis_config(max_value, plot_height, includes_pval0, "lower");
        var y_scale_female = y_axis_config_female.scale;

        // TODO: draw a small y-axis-break at 20 if `y_axis_config.draw_break_at_20`

        var y_axis_male = d3.axisLeft(y_scale_male)
            .tickFormat(d3.format("d"))
            .tickValues(y_axis_config_male.ticks);
        miami_plot.append("g")
            .attr("class", "y axis")
            .attr('transform', 'translate(-8,0)') // avoid letting points spill through the y axis.
            .call(y_axis_male);

        var y_axis_female = d3.axisLeft(y_scale_female)
            .tickFormat(d3.format("d"))
            .tickValues(y_axis_config_female.ticks);
        miami_plot.append("g")
            .attr("class", "y axis")
            .attr('transform', 'translate(-8,0)') // avoid letting points spill through the y axis.
            .call(y_axis_female);
        ...
        if (includes_pval0) {
            var y_axis_break_inf_offset_male = y_scale_male(Infinity) + (y_scale_male(0)-y_scale_male(Infinity)) * 0.03
        ...
        if (y_axis_config_male.draw_break_at_20) {
            var y_axis_break_20_offset = y_scale_male(20);
            miami_plot.append('line')
                .attr('x1', -8-7).attr('x2', -8+7)
                .attr('y1', y_axis_break_20_offset+6).attr('y2', y_axis_break_20_offset-6)
                .attr('stroke', '#666').attr('stroke-width', '3px');
        }
        if (y_axis_config_female.draw_break_at_20) {
            var y_axis_break_20_offset = y_scale_female(20);
            miami_plot.append('line')
                .attr('x1', -8-7).attr('x2', -8+7)
                .attr('y1', y_axis_break_20_offset+6).attr('y2', y_axis_break_20_offset-6)
                .attr('stroke', '#666').attr('stroke-width', '3px')
        ...
        if ( male_variant_bins != null){
            ...
            var color_by_chrom = d3.scaleOrdinal()
            .domain(get_chrom_offsets_male().chroms)
            .range(['rgb(160,20,20)', 'rgb(59,7,7)']);
        } else if ( female_variant_bins != null){
            var chroms_and_midpoints = (function() {
                var v = get_chrom_offsets_female();
                return v.chroms.map(function(chrom) {
                    return {
                        chrom: chrom,
                        midpoint: v.chrom_genomic_start_positions[chrom] + (v.chrom_extents[chrom][1] - v.chrom_extents[chrom][0]) / 2,
            ...
            var color_by_chrom = d3.scaleOrdinal()
            .domain(get_chrom_offsets_female().chroms)
            .range(['rgb(160,20,20)', 'rgb(59,7,7)']);
        }
        ...
        if (male_variant_bins != null){
            ...
            .attr('x1', 0)
            .attr('x2', plot_width)
            .attr('y1', y_scale_male(-Math.log10(significance_threshold)))
            .attr('y2', y_scale_male(-Math.log10(significance_threshold)))
            .attr('stroke-width', '5px')
            .attr('stroke', 'lightgray')
            .attr('stroke-dasharray', '10,10')
            .on('mouseover', significance_threshold_tooltip.show)
        ...
        if ( female_variant_bins != null){
            ...
            miami_plot.append('line')
                .attr('x1', 0)
                .attr('x2', plot_width)
                .attr('y1', y_scale_female(-Math.log10(significance_threshold)))
                .attr('y2', y_scale_female(-Math.log10(significance_threshold)))
                .attr('stroke-width', '5px')
                .attr('stroke', 'lightgray')
                .attr('stroke-dasharray', '10,10')
                .on('mouseover', significance_threshold_tooltip.show)
        ...
        // TODO: if the label touches any circles or labels, skip it?
        var variants_to_label_male = _.sortBy(_.where(male_unbinned_variants, {peak: true}), _.property('pval'))
            .filter(function(d) { return d.pval < 5e-8; })
            .slice(0,7);

        var variants_to_label_female = _.sortBy(_.where(female_unbinned_variants, {peak: true}), _.property('pval'))
        .filter(function(d) { return d.pval < 5e-8; })
        .slice(0,7);

        var genenames_male = miami_plot.append('g')
            .attr('class', 'genenames_male')
            .selectAll('text.genenames_male')
            .data(variants_to_label_male)
            .enter()
            .append('text')
            .attr('class', 'genename_text')
            .style('font-style', 'italic')
            ...
            .attr('transform', function(d) {
                return fmt('translate({0},{1})',
                            x_scale(get_genomic_position_male(d)),
                            y_scale_male(-Math.log10(d.pval))-5);
            })
            .text(function(d) {
                if (d.nearest_genes.split(',').length <= 2) {
                    return d.nearest_genes;
        ...
        var genenames_female = miami_plot.append('g')
            .attr('class', 'genenames_female')
            .selectAll('text.genenames_female')
            .data(variants_to_label_female)
            .enter()
            .append('text')
            .attr('class', 'genename_text')
            .style('font-style', 'italic')
            ...
            .attr('transform', function(d) {
                return fmt('translate({0},{1})',
                            x_scale(get_genomic_position_female(d)),
                            y_scale_female(-Math.log10(d.pval)) + 20);
            })
            .text(function(d) {
                if (d.nearest_genes.split(',').length <= 2) {
                    return d.nearest_genes;
        ...
        function pp1(flip) {
                if(flip ){
                    ...
                    .append('circle')
                    .attr('cx', function(d) {
                        return x_scale(get_genomic_position_female(d));
                    })
                    .attr('cy', function(d) {
                        return y_scale_female(-Math.log10(d.pval));
                    })
                    .attr('r', 7)
                    .style('opacity', 0)
                    .style('stroke-width', 1)
                    ...
                    .append('circle')
                    .attr('cx', function(d) {
                        return x_scale(get_genomic_position_male(d));
                    })
                    .attr('cy', function(d) {
                        return y_scale_male(-Math.log10(d.pval));
                    })
                    .attr('r', 7)
                    .style('opacity', 0)
                    .style('stroke-width', 1)
        ...
        function pp3(flip) { // drawing the ~60k binned variant circles takes ~500ms.  The (far fewer) unbinned variants take much less time.
            if(flip ) {
                ...
                .attr('class', 'bins')
                .selectAll('g.bin')
                .data(female_variant_bins)
                .enter()
                .append('g')
                .attr('class', 'bin')
                .attr('data-index', function(d, i) { return i; }) // make parent index available from DOM
                .each(function(d) { //todo: do this in a forEach
                    d.x = x_scale(get_genomic_position_female(d));
                    d.color = color_by_chrom(d.chrom);
                });
            bins.selectAll('circle.binned_variant_point')
                .data(_.property('qvals'))
                ...
                .attr('cx', function(d, i) {
                    var parent_i = +this.parentNode.getAttribute('data-index');
                    return female_variant_bins[parent_i].x;
                })
                .attr('cy', function(qval) {
                    return y_scale_female(qval);
                })
                .attr('r', 2.3)
                .style('fill', function(d, i) {
                    var parent_i = +this.parentNode.getAttribute('data-index');
                    return female_variant_bins[parent_i].color;
                })
            bins.selectAll('circle.binned_variant_line')
                .data(_.property('qval_extents'))
                .enter()
                ...
                .attr('y1', function(d) { return y_scale_female(d[0]); })
                .attr('y2', function(d) { return y_scale_female(d[1]); })
                .style('stroke', function(d, i) {
                    var parent_i = +this.parentNode.getAttribute('data-index');
                    return female_variant_bins[parent_i].color;
                })
                ...
                .attr('class', 'bins')
                .selectAll('g.bin')
                .data(male_variant_bins)
                .enter()
                .append('g')
                .attr('class', 'bin')
                .attr('data-index', function(d, i) { return i; }) // make parent index available from DOM
                .each(function(d) { //todo: do this in a forEach
                    d.x = x_scale(get_genomic_position_male(d));
                    d.color = color_by_chrom(d.chrom);
                });
            bins.selectAll('circle.binned_variant_point')
                .data(_.property('qvals'))
                ...
                .attr('cx', function(d, i) {
                    var parent_i = +this.parentNode.getAttribute('data-index');
                    return male_variant_bins[parent_i].x;
                })
                .attr('cy', function(qval) {
                    return y_scale_male(qval);
                })
                .attr('r', 2.3)
                .style('fill', function(d, i) {
                    var parent_i = +this.parentNode.getAttribute('data-index');
                    return male_variant_bins[parent_i].color;
                });
            bins.selectAll('circle.binned_variant_line')
                .data(_.property('qval_extents'))
                .enter()
                ...
                .attr('y1', function(d) { return y_scale_male(d[0]); })
                .attr('y2', function(d) { return y_scale_male(d[1]); })
                .style('stroke', function(d, i) {
                    var parent_i = +this.parentNode.getAttribute('data-index');
                    return male_variant_bins[parent_i].color;
                })
...
function calculate_qq_dimension(combined_data, male_data = null, female_data = null){

    var height = 0
    var width = 0
    var dimensions = [height, width]
    var all_data = [combined_data, female_data, male_data]
    if (male_data == null){
        var all_data = [combined_data, female_data]
    } else if (female_data == null){
        var all_data = [combined_data, male_data]
    }

    for (var qq_data of all_data){
...
function create_qq_plot(maf_ranges, qq_ci, height = null, width = null, type = "both") {
    ...
    $(function() {
        ...
        var qq_plot_container = '#qq_plot_container_' + type;
