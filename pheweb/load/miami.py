...
'''
This script creates json files which can be used to render Miami plots.
'''
...
def run(argv:List[str]) -> None:
    parser = argparse.ArgumentParser(description="Make a Miami plot for each phenotype which has a sex-stratified result.")
    ...
    parallelize_per_pheno(
        ...
        convert = make_miami_json_file,
        cmd = 'miami',
        phenos = phenos,
    )

def get_input_filepaths(pheno:dict) -> List[str]: return [get_pheno_filepath('pheno_gz', pheno['phenocode'])] #need to change this to male_female files somehow
def get_output_filepaths(pheno:dict) -> List[str]: return [get_pheno_filepath('miami', pheno['phenocode'], must_exist=False)]
...
def make_manhattan_json_file(pheno:Dict[str,Any]) -> None:
    make_manhattan_json_file_explicit(get_pheno_filepath('pheno_gz', pheno['phenocode']),
                                      get_pheno_filepath('miami', pheno['phenocode'], must_exist=False))
...
class Binner: #change binner to work with miami plot
    def __init__(self):
